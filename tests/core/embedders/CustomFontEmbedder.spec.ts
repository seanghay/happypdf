...
describe('CustomFontEmbedder', () => {
  ...
  it("exposes the font's name", async () => {
    const embedder = await CustomFontEmbedder.for(new Uint8Array(ubuntuFont));
  ...
  it('can apply variable font variations', async () => {
    const text = 'Variable Font Sample';
    const regular = await CustomFontEmbedder.for(
      googleSansVariableFont,
      undefined,
      undefined,
      { wght: 400 },
    );
    const bold = await CustomFontEmbedder.for(
      googleSansVariableFont,
      undefined,
      undefined,
      { wght: 700 },
    ...
    expect(regular.widthOfTextAtSize(text, 24)).not.toBeCloseTo(
      bold.widthOfTextAtSize(text, 24),
      3,
    );
  });
});
