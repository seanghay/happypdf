import { getEncodedTextRun } from '../core/embedders/EncodedText';
...
/**
 * Emits the operators needed to render a single encoded line. When the encoded
 * string carries a HarfBuzz-shaped text run, each glyph is positioned
 * individually so that marks, ligatures and reordering survive; otherwise we
 * fall back to a single `showText`.
 */
const drawEncodedText = (
  ...
  const glyphScale = asNumber(options.size) / 1000;
  ...
  for (let idx = 0, len = textRun.glyphs.length; idx < len; idx++) {
    ...
    operators.push(
      rotateAndSkewTextRadiansAndTranslate(
        ...
        toRadians(options.xSkew),
        toRadians(options.ySkew),
        asNumber(options.x) + (x + glyph.xOffset) * glyphScale,
        asNumber(options.y) + (y + glyph.yOffset) * glyphScale,
      ),
